...
| 日期 | 变更 |
|------|------|
| 2026-06-30 | **LLM 超时修复 + Chess 工具编排实验**：`core/llm_factory.py` OpenAI 构造加 `timeout=httpx.Timeout(600s, connect=30s)` + `max_retries=1`；`core/llm_client.py:chat` 加 try/except，API 异常返回 `{"error":...}` LLMResponse 而非 crash/hang。`core/agent_context.py` 新增 per-step call limit 机制：`call_limits` dict + `check_call_limit(tool_name)` + `reset_call_counters()`；`from_policy` 支持 `call_limits` 字段。`core/layers/base.py:_call_llm` 工具分发前检查 `ctx.check_call_limit()`，超限返回 error JSON 阻止执行。`core/env/chess_game_env.py:tool_policy` 放开：加入 `terminal/web_search/tavily_search/read_file/grep/kb_query/kb_modify/kb_fill_gap`，移除 `query_domain/create_domain`（未完成）；新增 `call_limits: {web_search: 2, tavily_search: 2}` 每步搜索硬限。`scripts/run_chess_experiment.py` 新增 `--group train/eval` + `--eval-elo` 参数；eval 固定 Elo 不自适应，train 观察 Elo 轨迹；Stockfish 启动检查（winget 安装路径自动探测 + PATH 注入）；prompt 加入工具编排引导 + stockfish UCI 命令模板（含全路径注入）；`_reset_step_counters` 每步重置 call counter。 |
| 2026-06-30 | **RoundTree/record_learning 修复**：`core/round_tree.py` 新增 `reset_round_history()` — 重建全局单例清空历史轮次，防止跨局决策树污染 `record_learning` 快照；`scripts/run_chess_experiment.py:_setup_cognitive` 每局开始时调用。`core/tools/record_learning_tool.py` 修当前轮上下文丢失：`_record_learning_handler` 在提交线程池前捕获 `current_node()`（threading.local 在工作线程不可见），`_build_and_save` 签名新增 `current_node=None` 参数，将未入 history 的当前轮 L1 节点（含已完成的 L2/L3 子节点）追加到 `tree_nodes`，使 LLM 观察者能看到触发学习的那一轮决策树。`core/tools/record_learning_tool.py` 修 pending 路径泄漏：新增 module-level `_pending_dir` + `set_pending_dir(path)` setter，`_build_and_save` 用全局 `_pending_dir` 替换硬编码 `Path("data/learning/pending")`；`core/chain_factory.py:_mount_tools` 构建链时调用 `set_pending_dir(data_root / "data" / "learning" / "pending")`，使隔离 fork 的学习数据写入 fork 内而非项目根 CWD。 |
...
| `reset_round_history` | `() → None` | 重建全局单例清空历史轮次（每局开始调用防跨局污染） | run_chess_experiment._setup_cognitive | — |
...
| 函数/类 | 签名 | 作用 | 上游调用者 | 下游调用 |
|----------|------|------|-----------|---------|
| `register_record_learning` | `(registry, pending_dir) → None` | Register record_learning tool (sync=false)；pending_dir 参数保留但未用（实际路径由 set_pending_dir 全局设置） | register_all_tools() | ToolRegistry.register |
| `set_pending_dir` | `(path: Path\|str) → None` | 设置 module-level _pending_dir，供隔离 fork 写入自己的 learning pipeline | chain_factory._mount_tools | — |
| `_record_learning_handler` | `(args) → str` | 提交前捕获 current_node()（threading.local 跨线程不可见），Submit to TaskRunner, return task_id；有 thread-local context 时登记到 SessionStore | ToolRegistry.dispatch | current_node(), TaskRunner.submit, _build_and_save, SessionStore.register_task (via get_task_context) |
| `_build_and_save` | `(domain, target, importance, reasoning, current_node=None) → dict` | Build stub → 将当前轮 L1 节点追加到 tree_nodes → LLM fills observations → write pending JSON 到 _pending_dir | _record_learning_handler | RoundTree.snapshot, current_node identity check, _fill_observations_llm |
