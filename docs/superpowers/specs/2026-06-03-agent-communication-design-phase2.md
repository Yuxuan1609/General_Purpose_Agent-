...
> **Phase 2 实施策略**: 初次执行时大面积留 stub，逐步填充。优先跑通 Reflect 主链路（Coordinator → ReflectionAgent 判责 → 归档），L2 Domain Graph、学习管道评分等复杂模块先 stub 后实现。
...
**L4 处理（TBD）**：现阶段不涉及 L4，暂定与 L3 相同挂载方式。后续根据实际需求确定是否需要独立结构。
...
每层 Manager 在 process() 中注入本层的结果上限提示词。**软性约束**——不硬截断，若 LLM 判定任务需求远超上限，应向上反馈并由 L1 拆解任务。
...
Manager prompt 写入位置: `core/layers/l0_5_1/manager.py` → `process()` 方法中注入 meta。

提示词: `"你有 20 条行为准则可用。规则超过此上限时，优先合并语义重叠的规则。" `
...
| 单张卡片长度 | ≤2000 字 | 软约束 |
| 单次检索卡片数 | ≤10 | 软约束 |
| 扩散检索步数 | 2 跳 | BFS 上限，硬约束 |

Manager prompt 写入位置: `core/layers/l2/manager.py` → `process()` 方法中注入 meta["l2_spec"]。

提示词: `"每次最多引用 10 张知识卡片（单张 ≤2000 字）。若任务明显需要超过 10 张卡片的信息，设置 over_limit: True 并向上层请求任务拆解。" `

### 10.3 L3 — 技能层

| 参数 | 值 | 说明 |
|------|-----|------|
| 单个 Skill 长度 | ≤2000 字 | 软约束 |
| 单次匹配 Skill 数 | ≤15 | 软约束 |

Manager prompt 写入位置: `core/layers/l3/manager.py` → `process()` 方法中注入 meta["l3_spec"]。

提示词: `"每次最多匹配 15 个技能（单个 ≤2000 字）。若任务明显需要超过 15 个技能，设置 over_limit: True 并向上层请求任务拆解。" `

### 10.4 超限处理流程

```
L3/L2 检测超限 → NOTIFY 中包含 over_limit: True + 需求估算
  → L1 收到超限 NOTIFY → 建议 Executor 拆解任务
  → Executor 将大 Task 拆为子 Task 序列

---

## 11. 学习工作流 (Learning Workflow)
...
Session 结束
  │
  ├─ TaskDecomposer: Session → LearningUnit[]（当前纯规则，未来 LLM + 专属 Agent）
  │
  ├─ 每个 LearningUnit:
  │   ├─ meta 在同一 LearningUnit 内的所有 ExecutionRecord 中保持一致
  │   ├─ LearningRefiner(LLM): 分析每步是否"值得学习"
  │   │   判断标准: 该步是否对达成 meta 中的目标有贡献
  │   │   未来: 判断标准本身可学习化
  │   │   输出: {worth_learning: [record_idx, ...], reasoning}
  │   │
  │   └─ 对 worth_learning 的记录:
  │       ├─ 发起 NOTIFY 给各层 ReflectionAgent
  │       └─ L1→L2→L3 层内反思 (investigate → fix)
  │
  └─ ReflectCoordinator._archive() → pending/ → learned/{domain}/
```

### 11.2 反思验证 (Reflection Verification) — 待实现

**核心问题**：修复后是否真的改善了？目前缺少闭环验证。

**应实现（后续 Phase）**：

| 验证方式 | 说明 |
|---------|------|
| **回归测试** | 对同一或相似场景重新执行，对比修复前后结果 |
| **统计验证** | 同类任务多次执行后，统计 success_count/failure_count 变化 |
| **置信度漂移检测** | L2 卡片的 confidence 变化趋势是否合理（不应暴涨暴跌） |
| **A/B 对比** | 保留修复前快照，新任务分配时随机选择新旧策略，对比效果 |
| **人工审核标记** | 高风险修复（如 L1 规则删除）需人工确认后才合并 |
| **验证失败回滚** | 若修复后效果下降，回滚到修复前状态并标记该修复为无效 |

**当前状态**：验证机制全部未实现。ReflectionAgent.fix() 直接写入，无反馈循环。

### 11.3 Executor 输出格式（Phase 2）

```
Executor.execute(obs):
  1. QUERY 链: L1→L2→L3
  2. collect_notify()
  3. 优先用 L1 NOTIFY.result 作为 action_text（L1 已完成推理）
  4. L1 无结果时 LLM fallback（仅格式适配，不替代推理）
  5. 写入 ExecutionRecord 到 pending/
```

TODO: 未来复杂任务格式可能需要额外一次 LLM 调用做格式化。
```
