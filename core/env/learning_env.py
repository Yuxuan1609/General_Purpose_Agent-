...
_CONSOLIDATION_FORMAT = (
    ...
    "## Output format (use @modify markup, NOT JSON)\n"
    "Output each modification as a separate line using @modify markup:\n\n"
    "  @modify layer=l2 type=deprecate target=card_id reason=\"merged into xxx\"\n"
    "  @modify layer=l2 type=create target=new_id content=\"merged strategy\" reason=\"merge of a,b,c\"\n"
    "  @modify layer=l1 type=create target=l1/rule_id content=\"new rule text\" reason=\"based on evidence\"\n\n"
    "Rules for @modify:\n"
    "- layer must be l1, l2, or l3\n"
    "- type must be create, update, or deprecate\n"
    "- target uses the original card/rule/skill ID\n"
    "- content and reason use double-quotes (use single-quotes inside if needed)\n"
    "- Each @modify on its own line\n"
    "- Prioritize: unused entries, low activation, highly redundant content\n"
    "- Prefer deprecate over delete (reversible)\n"
    "- Do NOT output JSON. Output ONLY @modify lines.\n"
)
