...
> **Note**: 当前 Execute 段**不进行多轮调用**。L1 Stage1 查询 L2 后直接进入 Stage2 决策（`MAX_LOOPS=1`），不会根据下层返回信息再次查询。后续需支持对话式多轮交互——L1 可多次向 L2 追问直到信息充分。此变更将影响通信协议、超时控制和日志格式。

### 执行路径（三轨，含架构演进）

> **当前架构认识**：Reflection 不应是写死的元操作，而是通过**独立环境（LearningEnv）** 将其降级为普通 domain，与 GameEnv 共享同一套层链 + ToolUse。以下同时保留旧和新两套描述。
...
```
  ...
  └─────────┬───────────┘  └──────────┬──────────────┘
            ...
            └──────────┬──────────────┘
                       ▼
            ┌──────────────────────┐
            │  Executor + Layers   │  ← Executor 不关心背后是什么环境
            │  (L(0.5+1)↔L2↔L3)   │     只做 observation → action
            └──────────────────────┘
```

- **GameEnv**: 原始认知任务环境（Leduc/DouZeroc），产生 `ExecutionRecord` → 入 LearningEnv
- **LearningEnv**: 读取 pending records，将学习建模为标准 env.step。状态 = 知识图谱快照，动作 = 提炼/编译/验证，奖励 = 下游胜率 delta
- **Executor**: 对两个环境完全无感，只发 `TaskObservation` 收 action。Orchestrator 控制切换哪个环境
- **ToolUse**: GameEnv 和 LearningEnv 共享。LearningEnv 可用工具查历史、跑假设验证、联网搜索——这突破了"反思不调工具"的隐含限制，可以实现非常高级的学习策略（回顾经验、主动构造反例、执行代码验证等）

**旧新架构执行路径对比**：

```
  ┌────────────────────────────────────────────────────┐
  │  PHASE 1.5: EXECUTE (Executor.execute)              │
  ...
  │  ────────────────────────────────────────────────── │
  │  ⚠ PHASE 2: 旧 REFLECT 设计（待重构为 LearningEnv）   │
...
最初设计的出发点是"不让 Execute 和 Reflect 交错"，防止 Executor 一边执行一边修改知识。这个出发点仍然成立，但实现方案发生了质变：

| 旧方案 | 新方案 |
|--------|--------|
| Reflect 是独立于 Execute 的第二阶段，写死判责逻辑 | Reflect 降级为 LearningEnv，和 GameEnv 平级 |
| 反思阶段 LLM 已停止，只读不写环境 | LearningEnv 的 LLM 正常工作，domain 约束而非阶段约束 |
| 反思不能调工具 | LearningEnv 可以调用 ToolUse——查历史、跑代码验证、联网搜索 |
| ReflectionAgent 写死每层判责逻辑 | 学习策略走普通层链，系统可以学到"如何学习"（自举） |

**核心不变**：同一个 Task 的 Execute 和 Reflect 不会同时发生——Orchestrator 确保时序。改变的是 Reflect **怎么做**——从"特殊第二阶段"变成了"一个独立的环境+普通 domain"。

**环境隔离（跨环境）：**

- GameEnv 阶段：Agent 通过 Executor + Layers 读写游戏状态，产出 ExecutionRecord
- LearningEnv 阶段：LearningEnv 消费 pending records，Agent 通过同一套 Executor + Layers 做知识操作
- 两个环境共享 ToolUse：LearningEnv 可用工具查历史、跑假设验证、执行代码验证等——这突破了"反思不调工具"的隐含限制
- **批量策略**仍保留，但由 LearningEnv 自己决定 batch 大小和反思时机：不硬编码 5-10 条阈值

```
Batch₁: Task₁ → Task₂ → ... → Task₅ → LearningEnv.step(domain="learning/reflect")
Batch₂: Task₆ → Task₇ → ... → Task₁₀ → LearningEnv.step(domain="learning/compile")
```
...
- A4 约束 **什么时候学**（Task 级别的 Execute→Reflect 循环，实现方式变更为 GameEnv ↔ LearningEnv 切换）
...
```
cognitive-agent/
  _archive/                    # 已归档旧架构代码（main.py、agent.py 等）
  config.yaml                  # 用户配置
