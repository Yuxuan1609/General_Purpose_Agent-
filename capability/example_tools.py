"""Example tools for validating the Capability system.

These tools follow the existing ToolRegistry.register() pattern
(OpenAI function-calling schema + handler function).
Stable tools should migrate to core/tools/ after validation.
"""
...
# Workspace root for path validation — defaults to project root,
...
def _validate_path(path_str: str) -> Path:
    """Resolve and validate a file path against workspace root.

    Rejects paths outside workspace or with dangerous patterns.
    """
...
def register_grep(registry):
    def handler(args=None, context=None):
        ...
        # Use ripgrep if available, fall back to Python glob + re
        try:
...
def _grep_python(search_dir: str, pattern: str, include: str) -> str:
    """Python fallback search using glob + re."""
    import re
